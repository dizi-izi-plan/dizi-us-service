...
Принимает данные нового пользователя (RegisterIn), создает учетную запись в системе и инициирует процесс подготовки профиля.

2. **Верификация почты (/auth/verify)**

Обрабатывает подтверждение адреса электронной почты пользователя. Принимает код или токен подтверждения (VerifyEmailIn) и активирует аккаунт.

3. **Авторизация (/auth/login)**

Выполняет вход пользователя в систему. Проверяет учетные данные (LoginIn) и возвращает пару токенов доступа (LoginOut).

4. **Обновление токена (/auth/refresh)**

Позволяет получить новую пару токенов без повторного ввода логина/пароля, используя действующий Refresh-токен.
